...
function Walkman({ title, next, prev, play, stop, isPlaying = false, language = 'pt' }) {
  const [seekState, setSeekState] = useState(null);
  const [isMinimized, setIsMinimized] = useState(false);
 
  const [lightsOn, setLightsOn] = useState(false);

  const toggleLabel = isMinimized
    ? language === 'en'
      ? "🎧 OPEN PLAYER"
      : "🎧 ABRIR PLAYER"
    : language === 'en'
  ...
  return (
    ...
    <div className={`walkman-wrapper ${isMinimized ? "minimized" : ""} ${lightsOn ? "lights-on" : ""}`}>
      
     
      <button 
        className="toggle-walkman-btn" 
        ...
        <main className="walkman-case-sony">
          <div className="silver-panel-left">
            <div className="side-button-group">
              ...
              <div 
                className="side-button vol-slider" 
                onClick={() => setLightsOn(!lightsOn)}
                style={{ cursor: "pointer" }}
                title="Alternar Luzes Internas"
              ></div>
